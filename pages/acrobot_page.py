...
def create_gif(frames, filepath="animation.gif", total_duration_s=5):
    ...
    total_duration_ms = total_duration_s * 1000  # Convert seconds to milliseconds
    frame_duration = total_duration_ms // len(
        frames
    )  # Floor division to get an integer result
...
def configure_agent():
    ...
    discount = st.sidebar.slider(
        "Discount Factor", min_value=0.8, max_value=0.99, value=0.95, step=0.01
    )
    eps = st.sidebar.slider("Episodes", min_value=10, max_value=1000, value=100)
...
def animate_agent(agent, episodes, total_duration_s=5):
    ...
    frames = agent.render(episodes=1)
    if not frames:
        raise ValueError("Rendering did not produce any frames.")
    gif_path = "temp/acrobot_animation.gif"
